...
function b(){ const fc=globalThis.document.getElementById("drop");const fd=globalThis.document.getElementById("myFile");const fe=globalThis.document.getElementById("File");const ff=globalThis.document.getElementById("mergeBtn");const fg=globalThis.document.getElementById("result");let fh=null;const fi=(fl) => { fl.preventDefault();fl.stopPropagation();};const fj=(fl) => { fh=i(fl);c().Clear();for(let fn=0, fo=fl.length-1;fn<=fo;fn++){ const fm=fl.item(fn);c().Append(fm.name);} };fc.addEventListener("dragover", fi);fc.addEventListener("dragenter", fi);fc.addEventListener("drop", (fl) => { fi(fl);return fj(fl.dataTransfer.files);});fd.addEventListener("change", () => fj(fd.files));fe.addEventListener("change", () => fj(fe.files));ff.addEventListener("click", () => { let fl;if(fh!=null&&fh.$==1&&(fh.$0.length>0&&(fl=fh.$0,true))){ const fm=Z(G(() => H((fo) => { const fp=null;return u0037(() => { const fq=fl.item(fo);return u0038(V(fq.arrayBuffer()), (fr) => { const fs=new Uint8Array(fr);const ft=q(fs.length, (fu) => fs[fu]);return bb([fq.name, ft]);});});}, l(0, fl.length-1))));const fn=null;return ba(u0037(() => { fg.innerText="Merging PDFs, please wait...";return u0038(u0039(fm), (fo) => u0038(p(Y(fo)), (fp) => { let fq;const fr=URL.createObjectURL(new Blob([new Uint8Array(fp)], (fq={},fq.type="application/pdf",fq)));const fs=globalThis.document.createElement("a");fs.setAttribute("href", fr);fs.setAttribute("download", "merged.pdf");fs.style.display="none";globalThis.document.body.appendChild(fs);fs.click();URL.revokeObjectURL(fr);globalThis.document.body.removeChild(fs);fg.innerText="PDFs merged successfully!";return $();}));}), null);} else { fg.innerText="Please select PDF files first.";return;} });const fk=X.Convert((fl) => X.Element("div", u0034.Empty, Y([X.TextNode(fl)])), c().v);bt("");X.RunById("fdisplay", fk);return X.Empty;}
